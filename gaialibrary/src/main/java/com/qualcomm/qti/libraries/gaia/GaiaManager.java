...
public abstract class GaiaManager {
    ...
    private void sendGAIAAcknowledgement(GaiaPacket packet, @GAIA.Status int status, @Nullable byte[] value) {
        ...
        try {
            ...
            //Log.d(TAG, "sendGAIAAcknowledgement: "+ ConvertUtils.bytes2HexString(bytes));
    ...
    protected void createRequest(GaiaPacket packet) {
        if (mShowDebugLogs) {
            Log.d(TAG, "Received request command: "
                    + GaiaUtils.getGAIACommandToString(packet.getCommand())+"data"+GaiaUtils.getHexadecimalStringFromBytes(packet.getPayload()));
        }
