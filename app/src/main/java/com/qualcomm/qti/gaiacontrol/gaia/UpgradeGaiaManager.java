...
public class UpgradeGaiaManager extends AGaiaManager implements UpgradeManager.UpgradeManagerListener {
     ...
     *
     * @param myListener An object which implements the {@link GaiaManagerListener MyGaiaManagerListener} interface.
     * @param transport  The type of transport this manager should use for the GAIA packet format:
     ...
     *
     * @param enable True to enable the display of the logs, false otherwise.
     */
     ...
     *
     * @param type         The type of confirmation the UpgradeManager requested.
     * @param confirmation To know if the UpgradeManager should continue the process or abort it.
     */
     ...
     *
     * @param enabled True to enable the RWCP mode, false to disable it.
     */
     ...
     *
     * @param payload The bytes which corresponds to a
    ...
    private void sendUpgradeControl(byte[] payload, boolean isTransferringData) {
        if (isTransferringData && mIsRWCPEnabled) {
            ...
            Log.d(TAG, "sendUpgradeControl: " + ConvertUtils.bytes2HexString(payload));
            GaiaPacket packet = createPacket(GAIA.COMMAND_VM_UPGRADE_CONTROL, payload);
            createRequest(packet);
        }
    }
     ...
     *
     * @param payload The payload to send with the command SET DATA ENDPOINT MODE.
     */
    ...
    // the parameter is always VMU_PACKET in this application
     ...
     *
     * @param packet The receive notification event packet.
    ...
    private boolean receiveEventNotification(GaiaPacket packet) {
        ...
        if (payload.length > 0) {
            ...
            if (event == GAIA.NotificationEvents.VMU_PACKET && mUpgradeManager != null) {
                ...
                Log.d(TAG, ConvertUtils.bytes2HexString(payload) + "receiveEventNotification: " + ConvertUtils.bytes2HexString(data));
    ...
    protected void receiveSuccessfulAcknowledgement(GaiaPacket packet) {
        Log.d(TAG, GAIA.COMMAND_VM_UPGRADE_CONTROL + "receiveSuccessfulAcknowledgement: " + packet.getCommand());
    ...
    public interface GaiaManagerListener {
         ...
         * </ul></p>
         *
         * @param error All the information relative to the occurred error.
         */
         ...
         *
         * @param percentage The percentage of the bytes of the file that have been sent to the board.
         */
         ...
         *
         * @param packet             The byte array to send to a device.
         * @param isTransferringData To know if the packet contains data to transfer.
         ...
         * </ul></p>
         *
         * @param enabled True if RWCP transfer mode is assumed supported and enabled by the device. False otherwise.
         */
