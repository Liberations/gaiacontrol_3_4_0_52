...
public class GAIAGATTBLEService extends BLEService implements BondStateReceiver.BondStateListener,
     ...
     *
     * @param size The MTU size to use when communicating with the connected device.
     ...
     *
     * @param size The size sets up by the user.
...
//        return mUpdateRssi;
    ...
    // the return value is used for some implementations
     ...
     *
     * @param message The type of message to send.
     * @param object  Any complementary information to send to the listener.
     */
    @SuppressWarnings("UnusedReturnValue")
     ...
     *
     * @param message    The type of message to send.
     * @param subMessage Any complementary message to the message type.
     * @param object     Any complementary object to send to the listener.
     */
    @SuppressWarnings({"UnusedReturnValue", "SameParameterValue"})
    ...
    protected void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        ...
        if (status == BluetoothGatt.GATT_SUCCESS && newState == BluetoothProfile.STATE_CONNECTED) {
            Log.i(TAG, "Attempting to start service discovery: " + "getBondState" + getDevice().getBondState());
            if (getDevice().getBondState() == BluetoothDevice.BOND_NONE) {
                ...
                Log.d(TAG, "discoverServices: "+gatt.discoverServices()+""+gatt.getServices());
            }
    ...
    public boolean refresh(BluetoothGatt gatt) {
        try {
    ...
    protected void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        ...
        if (status == BluetoothGatt.GATT_SUCCESS) {
            ...
            if (mShowDebugLogs)
                Log.i(TAG, "Successful write descriptor " + descriptorUuid.toString()
        ...
        if (status == BluetoothGatt.GATT_SUCCESS && mGattServices.gattServiceGaia.isSupported()
            ...
            if (mShowDebugLogs)
                Log.d(TAG, "Received successful onDescriptorWrite for Heart Rate Measurement");
    ...
    protected void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
        ...
        new Handler(getMainLooper()).post(new Runnable() {
            ...
            public void run() {
                if (mUpgradeGaiaManager == null) {
                    mUpgradeGaiaManager = new UpgradeGaiaManager(GAIAGATTBLEService.this, GAIA.Transport.BLE);
...
//        mGattServices.reset();
     ...
     * characteristic.</p>
     *
     * @param data The byte array to send to a device.
     * @return true if the sending could be done.
     */
    private boolean sendGaiaCommandEndpoint(byte[] data) {
        Log.d(TAG, "sendGaiaCommandEndpoint: " + ConvertUtils.bytes2HexString(data));
