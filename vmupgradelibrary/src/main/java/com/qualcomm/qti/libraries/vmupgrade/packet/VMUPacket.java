...
public class VMUPacket {
     ...
     *
     * @param opCode the operation code for this packet.
     * @param data   the date for this packet.
     */
     ...
     * </pre></blockquote></p>
     *
     * @param bytes The raw data of a VMU packet as sent by the device. To contain all mandatory information this bytes
    ...
    public VMUPacket(byte[] bytes) throws VMUException {
        if (bytes.length >= REQUIRED_INFORMATION_LENGTH) {
            ...
            //Log.d(TAG, "VMUPacket: after"+ ConvertUtils.bytes2HexString(bytes));
        } else {
