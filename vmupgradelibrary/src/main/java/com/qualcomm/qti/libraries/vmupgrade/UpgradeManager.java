...
public class UpgradeManager {
    ...
    public void onSuccessfulTransmission() {
        if (wasLastPacket) {
            ...
            if (mBytesToSend > 0 && mResumePoint == ResumePoints.Enum.DATA_TRANSFER && !mSendMultiplePackets) {
                //Log.d(TAG, "onSuccessfulTransmission: sendNextDataPacket");
                sendNextDataPacket();
            }else {
                //Log.d(TAG, "啥玩意");
            }
    ...
    private void sendVMUPacket(VMUPacket packet, boolean isTransferringData) {
        ...
        //String pacg = ConvertUtils.bytes2HexString(bytes);
        //Log.d(TAG, "sendVMUPacket: "+pacg);
        if (isUpgrading) {
            if (mShowDebugLogs)
    ...
    private void sendSyncReq () {
        ...
        //Log.d(TAG, ConvertUtils.bytes2HexString(md5Checksum)+"sendSyncReq: "+ ConvertUtils.bytes2HexString(data));
    ...
    private void sendTransferCompleteReq(boolean process) {
       ...
       // Log.d(TAG, "sendTransferCompleteReq: "+ConvertUtils.bytes2HexString(packet.getBytes()));
        sendVMUPacket(packet, false);
    }
    ...
    private void receiveStartCFM(VMUPacket packet) {
        byte[] data = packet.getData();
       // Log.d(TAG, "receiveStartCFM: "+ConvertUtils.bytes2HexString(packet.getData()));
    ...
    private void receiveDataBytesREQ(VMUPacket packet) {
        ...
        if (data.length == OpCodes.UpgradeDataBytesREQ.DATA_LENGTH) {
            ...
            //Log.d(TAG, ConvertUtils.bytes2HexString(data)+"receiveDataBytesREQ: fileOffset"+fileOffset+"mBytesToSend"+mBytesToSend);
            ...
            //Log.d(TAG, "wenTest receiveDataBytesREQ: "+mBytesToSend);
            if (mSendMultiplePackets) {
                while (mBytesToSend > 0) {
                    Log.d(TAG, "mSendMultiplePackets: sendNextDataPacket");
                    sendNextDataPacket();
                }
            }
            else {
                //Log.d(TAG, "receiveDataBytesREQ: sendNextDataPacket");
                sendNextDataPacket();
            }
        }
        else {
