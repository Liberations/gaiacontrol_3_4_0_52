...
public abstract class BLEService extends Service {
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            //Log.d(TAG, "wenTest read: uuid" + characteristic.getUuid() + "\n" + ConvertUtils.bytes2HexString(characteristic.getValue()));
        }

        @Override
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            onReceivedCharacteristicChanged(gatt, characteristic);
            //Log.d(TAG, "wenDataRec>" + ConvertUtils.bytes2HexString(characteristic.getValue()));
        }
     ...
     *
     * @param delay The new delay time to use.
     */
     ...
     *
     * @param characteristicUUID the specified characteristic UUID.
    ...
    protected boolean requestCharacteristicNotification(BluetoothGattCharacteristic characteristic, boolean register) {
        if (mShowDebugLogs) {
            Log.d(TAG, characteristic.getService().getUuid().toString() + "Request received for notification on characteristic with UUID "
     ...
     *
     * @param characteristicUUID the specified characteristic.
    ...
    protected boolean requestWriteCharacteristic(BluetoothGattCharacteristic characteristic, final byte[] data) {
        if (mShowDebugLogs) {
            Log.d(TAG, characteristic.getService().getUuid() + "Request received for write on characteristic with UUID " + characteristic.getUuid().toString());
        }
        //Log.d(TAG, "wenDataWrite>" + ConvertUtils.bytes2HexString(data));
        if (mConnectionState != State.CONNECTED) {
            Log.w(TAG, "request write characteristic not initiated: device is disconnected.");
            return false;
        }
    ...
    byte[] data) {
        ...
        //Log.d(TAG, "wenDataWrite2>" + ConvertUtils.bytes2HexString(data));
        // all check passed successfully, the request can be initiated
     ...
     *
     * @param characteristicUUID Give a specified characteristic which requires pairing to be used.
     * @return <p>false if it is not possible to request a characteristic read. The reasons could be:
     * <ul>
     *     <li>The device is not connected.</li>
     ...
     * most stable request as it is less parameter dependant than a write request for instance.</p>
     *
     * @param characteristic Give a specified characteristic which requires pairing to be used.
     * @return <p>false if it is not possible to request a characteristic read. The reasons could be:
     * <ul>
     *     <li>The device is not connected.</li>
     *     <li>The given characteristic UUID does not correspond to any of the available characteristics of the
     ...
     * Bluetooth device.</p>
     *
     * @param descriptor the specified descriptor.
     ...
     *
     * @param mtu The size to start the negotiations with.
     ...
     *
     * @param address The device to connect with.
    ...
    protected boolean connectToDevice(BluetoothDevice device) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            mBluetoothGatt = device.connectGatt(this, false, mGattCallback, BluetoothDevice.TRANSPORT_LE, BluetoothDevice.PHY_LE_1M, new Handler());
// 设置gatt_is_robust_caching_enabled属性为禁用状态
     ...
     *
     * @param gatt     The Bluetooth gatt concerned for the connection state change.
     * @param status   The status of this connection change: {@link BluetoothGatt#GATT_SUCCESS} if the operation succeeds.
     * @param newState The new connection state: Can be one of {@link BluetoothProfile#STATE_DISCONNECTED} or {@link BluetoothProfile#STATE_CONNECTED}.
     */
     ...
     *
     * @param gatt   The Bluetooth gatt concerned for the services discovering.
     * @param status The status of the service discovering: {@link BluetoothGatt#GATT_SUCCESS} if the operation succeeds.
     */
    @SuppressWarnings({"EmptyMethod", "UnusedParameters"})
     ...
     *
     * @param gatt           The Bluetooth gatt which received the information.
     * @param characteristic The Bluetooth Characteristic for which there is a received information.
     * @param status         The status of this characteristic reading: {@link BluetoothGatt#GATT_SUCCESS} if the operation succeeds.
     */
    @SuppressWarnings({"EmptyMethod", "UnusedParameters"})
     ...
     *
     * @param gatt           The Bluetooth gatt which received the information.
     * @param characteristic The Bluetooth Characteristic for which there is a change.
     */
    @SuppressWarnings({"EmptyMethod", "UnusedParameters"})
     ...
     * @param status     The result of the write operation {@link BluetoothGatt#GATT_SUCCESS} if the operation succeeds.
     */
    @SuppressWarnings({"EmptyMethod", "UnusedParameters"})
     ...
     * @param rssi   The rssi value sent by the remote device.
     * @param status The result of the read operation, {@link BluetoothGatt#GATT_SUCCESS} if the operation succeeds.
     */
    @SuppressWarnings({"EmptyMethod", "UnusedParameters"})
     ...
     * <p>This method is called when a descriptor write operation has been requested</p>
     *
     * @param gatt       The Bluetooth gatt which requested the descriptor read operation.
     * @param descriptor The Bluetooth Characteristic Descriptor for which a request has been made.
     * @param status     The result of the read operation {@link BluetoothGatt#GATT_SUCCESS} if the operation succeeds.
     */
    @SuppressWarnings({"EmptyMethod", "UnusedParameters"})
     ...
     *
     * @param gatt           The Bluetooth gatt which requested the characteristic write operation.
     * @param characteristic The Bluetooth Characteristic Characteristic for which a request has been made.
     * @param status         The result of the write operation {@link BluetoothGatt#GATT_SUCCESS} if the operation succeeds.
     */
    @SuppressWarnings({"EmptyMethod", "UnusedParameters"})
    ...
    private void receiveConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        ...
        if (status == BluetoothGatt.GATT_SUCCESS && newState == BluetoothProfile.STATE_CONNECTED) {
            ...
            if (mBluetoothGatt == null) {
                mBluetoothGatt = gatt;
                BluetoothGattCharacteristic characteristic = gatt.getService(UUID.fromString("00009cf1-0000-1000-8000-00805f9b34fb")).getCharacteristic(UUID.fromString("0000f103-0000-1000-8000-00805f9b34fb"));
     ...
     *
     * @param gatt   The Bluetooth gatt which received the information.
     * @param rssi   The rssi value which had given.
     * @param status The status of this characteristic reading: {@link BluetoothGatt#GATT_SUCCESS} if the operation succeeds.
     */
     ...
     *
     * @param gatt   The Bluetooth gatt which received the information.
     * @param mtu    The mtu value which had been chosen.
     * @param status The status of this characteristic reading: {@link BluetoothGatt#GATT_SUCCESS} if the operation
     *               succeeds.
     */
     ...
     *
     * @param descriptor the descriptor to request a read operation on.
     * @return true, if the read operation was initiated successfully.
     */
     ...
     *
     * @param descriptor the descriptor to request a write operation on.
     * @return true, if the read operation was initiated successfully.
     */
     ...
     *
     * @param characteristic the characteristic to request a write operation on.
     * @return true, if the read operation was initiated successfully.
     */
    private boolean writeCharacteristic(BluetoothGattCharacteristic characteristic) {
        if (mShowDebugLogs) {
            //Log.d(TAG, ConvertUtils.bytes2HexString(characteristic.getValue()) + "Process request write characteristic for characteristic " + characteristic.getUuid());
        }
        if (mBluetoothAdapter == null) {
     ...
     * </ul></p>
     *
     * @param request The request which has failed.
     */
     ...
     *
     * @param requestType    The type of request which received a callback.
     * @param characteristic The characteristic of the callback.
     ...
     *
     * @param request The request item to add to the queue.
     */
